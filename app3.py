...
KAYITLI_PLAKALAR = ["16 LBT 68", "34 NDH 928"]
...
# Kamera bağlantısı
cap = cv2.VideoCapture("http://192.168.215.221:8080/video")
        ...
        for r in results:
            for box in r.boxes:
                ...
                if cls_name == "plaka":
                    ...
                    if ocr_result:
                        plate_text = ocr_result[0][1].strip().upper()
                ...
                # Komut mantığı (hem kamyonet hem araba için)
                elif cls_name in ["kamyonet", "araba"]:
                    plaka_normalize = current_plate.replace(" ", "").upper()
                    kayitli_mi = any(plaka_normalize == p.replace(" ", "").upper() for p in KAYITLI_PLAKALAR)

                    if cls_name == "kamyonet":
                        if kayitli_mi:
                            komut_gonder("KAPI")
                        else:
                            komut_gonder("BUZZER")
